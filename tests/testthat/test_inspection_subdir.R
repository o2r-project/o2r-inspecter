...
library("httr")
library("jsonlite")
context("/api/v1/inspection - files from subdirs")
...
test_that("/api/v1/inspection/WQI9V?file=subdir/classes.RData gives HTTP 200 with valid JSON)", {
  r <- GET("http://localhost:8091/api/v1/inspection/WQI9V?file=subdir/classes.RData")
  expect_equal(status_code(r), 200)
  expect_true(jsonlite::validate(httr::content(r, as = "text", encoding = "UTF-8")))
})
